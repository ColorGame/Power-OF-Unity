...
public class MouseOnGameGrid : MonoBehaviour // Класс отвечающий за положение курсора мыши в сетке
{
   ...
   private GridPositionXZ _mouseGridPosition;  // сеточная позиция мыши
    ...
    private void Awake()
    {
        ...
        {
            Debug.LogError("There's more than one MouseOnGameGrid!(Там больше, чем один MouseOnGameGrid!) " + transform + " - " + Instance);
    ...
    private void Start()
    {
        _mouseGridPosition = LevelGrid.Instance.GetGridPosition(GetPositionOnlyHitVisible());  // Установим при старте сеточную позицию мышм // НЕЛЬЗЯ ВЫЗЫВАТЬ в Awake() т.к. в результате гонки возникает нулевая ошибка (кто проснется раньше GameInput или MouseOnGameGrid неизвестно)
    }

    // Для теста, светящий шар следует за курсором мыши.
    /*private void Update()
    {
        transform.position = MouseOnGameGrid.GetPosition(); // Так можно вызывать из ЛЮБОГО МЕСТА
    }*/
