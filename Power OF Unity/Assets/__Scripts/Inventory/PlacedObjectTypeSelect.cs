        ...
        foreach (PlacedObjectTypeSO placedObjectTypeSO in _placedObjectTypeListSO.list) // Переберем список Типов Размещаемых объектов
        {
            ...
            switch (placedObjectTypeSO) // В зависимости от типа поместим в нужный контейнер
            {
                case WeaponTypeSO weaponTypeSO:
                    buttonTransform = Instantiate(GameAssets.Instance.placedObjectTypeButtonPrefab, _weaponSelectContainer); // Создадим кнопку и сделаем дочерним к контенеру
                    break;

                case ItemTypeSO itemTypeSO:
                    buttonTransform = Instantiate(GameAssets.Instance.placedObjectTypeButtonPrefab, _itemSelectContainer); // Создадим кнопку и сделаем дочерним к контенеру
                    break;

                case ModuleTypeSO moduleTypeSO:
                    buttonTransform = Instantiate(GameAssets.Instance.placedObjectTypeButtonPrefab, _moduleSelectContainer); // Создадим кнопку и сделаем дочерним к контенеру
                    break;
            }
