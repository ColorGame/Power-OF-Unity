...
public class PickUpDropSystem : MonoBehaviour // Поднятие Перетаскивание и Бросание объектовЫ
{
    ...
    private void Awake()
    {
        if (Instance != null) // Сделаем проверку что этот объект существует в еденичном екземпляре
        {
            ...
            return; // т.к. у нас уже есть экземпляр PickUpDropSystem прекратим выполнение, что бы не выполнить строку ниже
        }
        Instance = this;
    }
    ...
    private void Start()
    {       
        _plane = new Plane(_canvasInventoryWorld.forward, _canvasInventoryWorld.position); // Создадим плоскость позиции canvasInventory
    }
    ...
    public void TryGrab()
    {
        Ray ray = _cameraUI.ScreenPointToRay(InputManager.Instance.GetMouseScreenPosition()); //Возвращает луч, идущий от камеры через точку экрана где находиться курсор мыши 
    ...
    /*  public Vector3 CalculateOffsetGrab() // вычислим смещение захвата
      ...
      {
          Ray ray = _cameraUI.ScreenPointToRay(InputManager.Instance.GetMouseScreenPosition()); // Луч от камеры в точку на экране где находиться курсор мыши
