...
/// <summary>
/// Система поиска пути. Будет запускаться при включении PathfindingParams.<br/>Работать с данной системой через PathfindingProvider
/// </summary>
...
partial struct PathfindingSystem : ISystem
{
    ...
    /* public void OnUpdate(ref SystemState state)
     {
                 ...
                 RefRO<PathfindingGridDate>>())
         {
             ...
             PathNodeSystem.DefaultPathNodeArray PathNodeDefaultEntityArray = SystemAPI.GetSingleton<PathNodeSystem.DefaultPathNodeArray>(); //Прикриплен к сущности системы поэтому получаем через синголтон
             NativeArray<PathNode> pathNodeTempArray = new NativeArray<PathNode>(PathNodeDefaultEntityArray.pathNodeTempArray, Allocator.Temp); // Временный массив Скопируем дефолтный массив
    ...
    public void OnUpdate(ref SystemState state)
    {
        //Запуститься при включенной PathfindingParams и любым состоянием PathfindingGridDate
        foreach ((
            RefRW<PathfindingParams> pathfindingParams,
            RefRO<PathfindingGridDate> pathfindingGridDate,
            ...
            in SystemAPI.Query<
                RefRW<PathfindingParams>,
                RefRO<PathfindingGridDate>>().WithPresent<PathfindingGridDate>().WithEntityAccess())
        {

            DefaultPathNodeArray PathNodeDefaultEntityArray = SystemAPI.GetSingleton<DefaultPathNodeArray>(); //Прикриплен к сущности системы поэтому получаем через синголтон
            ...
            for (int x = -moveDistance; x <= moveDistance; x++) // Юнит это центр нашей позиции с координатами unitGridPosition, поэтому переберем допустимые значения в условном радиусе maxMoveDistance
            {
                for (int y = -moveDistance; y <= moveDistance; y++)
                {
                    for (int z = -moveDistance; z <= moveDistance; z++)
                    {
                        ...
                        NativeArray<PathNode> pathNodeTempArray = new NativeArray<PathNode>(PathNodeDefaultEntityArray.pathNodeArray, Allocator.Temp);
            ...
            //Запустим событие у провайдера для MonoBehaviour
            PathfindingProvider.Instance.PathfindingComplete(validGridPositionPathNodeDict.dictionary);
