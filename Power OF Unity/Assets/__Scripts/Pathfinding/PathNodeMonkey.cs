...
public class PathNodeMonkey // Узел поиска пути // Будет создается в каждой ячейки сетки, И будет хранить в себе частные данные для расчета поиска пути G H F
                      ...
                      // класс нападоби GridObjectUnitXZ
{
    ...
    private PathNodeMonkey _cameFromPathNode; // Получено из PathNodeMonkey // В каждой ячейки нам нужна ссылка на ячейку откуда мы прибыли
    private bool _isWalkeble = true; // Можно ходить // Для настроики непроходимых узлов

    public PathNodeMonkey(GridPositionXZ gridPosition) // Конструктор
    {
    ...
    public void SetCameFromPathNode(PathNodeMonkey pathNode) // Установить пришел  С Узла Пути
    {
        _cameFromPathNode = pathNode;
    }

    public PathNodeMonkey GetCameFromPathNode() // Получить пришел  С Узла Пути
    {
