...
{
    public GridSystemTiltedXY(float canvasScaleFactor, InventoryGridParameters gridParameters, Func<GridSystemXY<TGridObject>, Vector2Int, TGridObject> createGridObject) : base(canvasScaleFactor,gridParameters, createGridObject)
    {
    }
    ...
    public override Vector3 GetWorldPositionGridCenter() //  Получим мировые координаты центра Сетки (относительно  _anchorGridTransform)
    {
        ...
        return _anchorGridTransform.TransformPoint(gridCenter); // transform.TransformPoint -Преобразует position нашей ячейки из локального пространства(_anchorGridTransform) в мировое пространство(учитывая наклон поворот _anchorGridTransform). т.к. _anchorGridTransform может находиться не в (0,0) и наклонен в мировом пространмтве
    }
