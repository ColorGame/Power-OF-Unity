...
{
    public static LevelGridVisual Instance { get; private set; }   //(ПАТТЕРН SINGLETON) Это свойство которое может быть заданно (SET-присвоено) только этим классом, но может быть прочитан GET любым другим классом
    ...
    private void Start()
    {
        ...
        for (int x = 0; x < LevelGrid.Instance.GetWidth(); x++)
        {
            for (int z = 0; z < LevelGrid.Instance.GetHeight(); z++)
            {
                for (int floor = 0; floor < LevelGrid.Instance.GetFloorAmount(); floor++)  // переберем все этажи
                {
                    ...
                    _gridSystemVisualSingleArray[x, z, floor] = gridSystemVisualSingleTransform.GetComponent<LevelGridVisualSingle>(); // Сохраняем компонент LevelGridVisualSingle в трехмерный массив где x,y,floor это будут индексы массива.
                }
            }
        }
        ...
        MouseOnGameGrid.OnMouseGridPositionChanged += MouseWorld_OnMouseGridPositionChanged;// подпишемся на событие Сеточная Позиция Мыши Изменена для включения и выключения круга показ. диапазон поражения гранаты
    ...
    /*private void Update()
    {
        ...
        {
            _lastSelectedGridSystemVisualSingle.HideSelected(); // Спрячим последний выбранный LevelGridVisualSingle
        }

        Vector3 mouseWorldPosition = MouseOnGameGrid.GetPosition(); //Мировая позиция мыши
        ...
        {
            _lastSelectedGridSystemVisualSingle = _gridSystemVisualSingleArray[_gridPositioAnchor.x, _gridPositioAnchor.y]; // Сохраним наш захваченый LevelGridVisualSingle
        }

        if (_lastSelectedGridSystemVisualSingle != null)
        {
            _lastSelectedGridSystemVisualSingle.ShowSelected();// Покажем последний выбранный LevelGridVisualSingle
        }
    }*/

    private void MouseWorld_OnMouseGridPositionChanged(object sender, MouseOnGameGrid.OnMouseGridPositionChangedEventArgs e)
    {
