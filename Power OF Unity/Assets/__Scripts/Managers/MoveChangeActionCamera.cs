...
public class MoveChangeActionCamera : MonoBehaviour // перемещение и переключение экшен камеры
{
    [SerializeField] private GameObject _actionCamera; // В инспекторе закинем ActionVirtualCamera и НАСТРОИМ на ней в инспекторе приоритет Priority = 20// На основной камере в разделе Cinemachine настроить время перехода Defauld Blend = 0.2
    ...
    private void Instance_OnInteractSphereAndDoor(object sender, DoorInteract e)
    {
        ...
        _actionCamera.transform.position = actionCameraDoorPosition;// Переместим нашу экшен камеру в настроенную позицию
        _actionCamera.transform.LookAt(transformDoor.position + Vector3.up * 2f); // Посмотрим навысоту 2 в сторону двери 
    ...
    private void BaseAction_OnAnyActionStart(object sender, System.EventArgs e)
    {
        ...
        switch (sender)
        {
            case ShootAction shootAction:
                ...
                _actionCamera.transform.position = actionCameraPosition; // Переместим нашу экшен камеру в настроенную позицию
                _actionCamera.transform.LookAt(targetUnit.GetWorldPosition() + cameraCharacterHeight); // Посмотрим в сторону врага на высоту cameraCharacterHeight (startUnit.GetWorldPositionCenterСornerCell() вернет позицию у основания ног)
            ...
            case SwordAction swordAction:
                ...
                _actionCamera.transform.LookAt(targetUnit.GetWorldPosition() + Vector3.up * 1f); // Посмотрим в сторону врага на высоту плеча (startUnit.GetWorldPositionCenterСornerCell() вернет позицию у основания ног)
