...
public class LevelScripting : MonoBehaviour // Сенарий уровня. Отвечает за взаимодействие (дверей и врагов) (шара-взаимодействия и дверей) (шара-взаимодействия и бочки-взаимодействия)
{
    public static LevelScripting Instance { get; private set; }   //(ПАТТЕРН SINGLETON) Это свойство которое может быть заданно (SET-присвоено) только этим классом, но может быть прочитан GET любым другим классом
                                                                  // _instance - экземпляр, У нас будет один экземпляр LevelScripting можно сдел его static. Instance нужен для того чтобы другие методы, через него, могли подписаться на Event.
    ...
    private void InteractSphereAndDoor(DoorInteract doorInteract) // Взаимодействуют сфера и дверь
    {
        ...
        OnInteractSphereAndDoor?.Invoke(this, doorInteract); // Запустим событие Сфера и Дверь Взаимодействуют (подписчик MoveChangeActionCamera для переключения камеры на дверь) // В аргумент передадим дверь с которой происходит взаимодействие
    }
